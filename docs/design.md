...
> Agent Relay Control · v1.0 设计基线 · 2026-09-17
>
...
后续工作可以围绕三个具体交付推进，尚未排期：

1. 完善插件执行契约与版本兼容矩阵，降低新界面的接入成本。
2. 用“本地选定邮件 → 云端公开调研 → 本地回收结果”验证受限委派，先定义材料范围、接收方、提交与审计，再实现协议。
3. 为信息状态同步建立独立基准，比较保真、权限和成本，不让研究假设悄悄变成 v1 的隐含承诺。
